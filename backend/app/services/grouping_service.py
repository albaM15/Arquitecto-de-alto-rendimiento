...
def _risk_score(student: StudentInput) -> float:
    ...
    return round(
        failures + math.log1p(absences) + (1.0 if g1 < 10 else 0.0),
        3
    )
...
def generate_groups(request: GroupGenerationRequest) -> GroupGenerationResponse:
    ...
    El Random Forest predice el perfil de cada estudiante.
    Luego esta función aplica reglas de negocio para mezclar perfiles complementarios.
    """
    ...
    for bucket in buckets.values():
        bucket.sort(
            key=lambda item: (item.risk_score, -item.G1),
            reverse=True
        )

    for profile_id in sorted(
        buckets.keys(),
        key=lambda pid: len(buckets[pid]),
        reverse=True
    ):
        for member in buckets[profile_id]:
            candidates = [
                ...
                if len(group) < request.group_size
            ]
            ...
            def score_group(index: int) -> tuple[int, float, int]:
                ...
                avg_g1 = (
                    sum(item.G1 for item in group) / len(group)
                    if group
                    else 0.0
                )
                ...
                same_profile_count = sum(
                    1
                    for item in group
                    if item.profile_id == member.profile_id
                )
    ...
    for index, group in enumerate(groups, start=1):
        ...
        avg_g1 = round(
            sum(member.G1 for member in group) / len(group),
            2
        )
        ...
        risk_count = sum(
            1
            for member in group
            if member.risk_score >= 2.0
        )

        explanation = (
            f"Grupo con promedio académico G1={avg_g1}. "
            f"Incluye {len(distribution)} perfil(es) distinto(s) y "
            f"{risk_count} estudiante(s) con riesgo académico alto. "
